        ...
        while (true) {
            ...
            Effect.gen(function* () {
              ...
              if (flags.experimentalEventSystem) {
                ...
                yield* events.publish(SessionEvent.Step.Failed, {
                  sessionID,
                  assistantMessageID,
                  error: { type: "unknown", message: errorMessage(error) },
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            const [env, instructions] = yield* Effect.all([
              ...
              sparse
                ? instruction.systemScoped("project", reader).pipe(Effect.orDie)
                : instruction.system(reader).pipe(Effect.orDie),
            ])

            // Sparse discards skills and MCP docs, so branch before building them rather than after.
            ...
            LLM.rejoinSystemForCaching(system, header)
            yield* plugin.trigger(
